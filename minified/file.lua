...
g,k=x[2],x[3]end end SkyOS.sLog.info("[info] offset "..g.."X, "..k
"Y")SkyOS.sLog.info("[info] loading image")for E=1,e.countLines(w),1 do local
T=v.readLine()local A=e.split(T,",")local O=A[1]local I=tonumber(A[2])+g local
N=tonumber(A[3])+k if O=="P"then
